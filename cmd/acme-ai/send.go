...
func sendLLMRequest(provider llm.Provider, win *acme.Win, winID int, conv *conversation.Conversation) error {
	...
	// Clear out the existing [[AiSend]]
	err = win.Addr("/ \\[\\[AiSend\\]\\]")
	if err != nil {
		return fmt.Errorf("failed to set address for clearing AiSend prompt: %w", err)
	}

	_, err = win.Write("data", []byte(""))
	if err != nil {
		return fmt.Errorf("failed to cleare previous AiSend prompt: %w", err)
	}

	// Select the ... thinking..., we will then overwrite that with the
	// response that we have received
	err = win.Addr("/... thinking...")
	if err != nil {
